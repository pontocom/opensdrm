...
class MDS_Administrator extends DatabaseHandler
{
	var $db_con;
	...
	function verifyLogin($username, $password){
		$rs=$this->db_con->executeQuery("SELECT * FROM mdsWS_Admin WHERE login='$username' AND password='$password'");
		/* verificar resultado */
	...
	function verifyLogin_($username, $password){
		$rs=$this->db_con->executeQuery("SELECT * FROM mdsWS_Admin WHERE login='$username' AND password='".md5($password)."'");
		/* verificar resultado */
		if($rs->RecordCount()==0) return false;
		return true;
	}
	...
	function verifyAdminStatus() {
		$rs=$this->db_con->executeQuery("SELECT COUNT(*) AS admin FROM mdsWS_Admin");
		/* verify if true */
	...
	function createNewAdminAccount($username, $password, $ksize) {
		...
		$parameters = array (
			"arbitrary_data" => "<name>Media Delivery Server</name>",
		...
		$sql = "INSERT INTO mdsWS_Admin VALUES ('$username', '$password', '".$this->pubkey->returnXMLkey()."', '".$this->privkey->returnXMLkey()."', '".$result["certificate"]."')";
	...
	function createNewAdminAccount_($username, $password) {
		...
		$mycsr->createCSR(array("countryName" => 'PT', "stateOrProvinceName" => 'Lisboa', "localityName" => 'Lisboa', "organizationName" => 'ISCTE', "organizationalUnitName" => 'ADETTI', "commonName" => 'MDS', "emailAddress" => 'mds@adetti.pt'),$private_key);
		...
		/* inserts all data on the table */
		$sql = "INSERT INTO mdsws_admin VALUES ('$username', '".md5($password)."', '".$private_key->_spKey."', '".$result["certificate"]."')";
		$rs=$this->db_con->executeQuery($sql);
		if($rs==false) {
			$this->error = "ERROR: An error has occured while trying to insert data on the database.<br>";
			return false;
	...
	function getData() {
		$rs=$this->db_con->executeQuery("SELECT * FROM mdsWS_Admin");
		/* verificar resultado */
		if($rs->RecordCount()==0) return false;
		else {
	...
	function exportData() {
		CreateFile($_SERVER["DOCUMENT_ROOT"].$GLOBALS['INSTALLATION_DIR']."/MDS/admin/exports/publickey.xml", $this->pubkey->returnXMLKey());
		$aes = new AESEncrypt();
		CreateFile($_SERVER["DOCUMENT_ROOT"].$GLOBALS['INSTALLATION_DIR']."/MDS/admin/exports/privatekey.xml.enc", $aes->AES_encrypt($this->privkey->returnXMLKey(), $this->password));
		CreateFile($_SERVER["DOCUMENT_ROOT"].$GLOBALS['INSTALLATION_DIR']."/MDS/admin/exports/certificate.xml", $this->cert->returnXMLCertificate());
	}
...
<table width=\"500\" border=\"1\" cellspacing=\"0\" cellpadding=\"0\">
  ...
  <tr>
	 ...
	 */
	function closeMDS_Administrator()
	{
