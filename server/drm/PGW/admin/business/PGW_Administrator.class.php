...
class PGW_Administrator extends DatabaseHandler
{
	var $db_con;
	var $login;
	var $password;
	 ...
	 */
	function PGW_Administrator()
	{
		global $PGW_DB_SERVER, $PGW_DB_DBNAME, $PGW_DB_USRNAME, $PGW_DB_PWD;
		$this->db_con = new DatabaseHandler();
		$this->db_con->connectTo($PGW_DB_SERVER, $PGW_DB_DBNAME, $PGW_DB_USRNAME, $PGW_DB_PWD);
	}
	...
	function verifyLogin($username, $password){
		$rs=$this->db_con->executeQuery("SELECT * FROM pgwWS_Admin WHERE login='$username' AND password='$password'");
		/* verificar resultado */
	...
	function verifyLogin_($username, $password){
		$rs=$this->db_con->executeQuery("SELECT * FROM pgwWS_Admin WHERE login='$username' AND password='".md5($password)."'");
		/* verificar resultado */
		if($rs->RecordCount()==0) return false;
		return true;
	}
	...
	function verifyAdminStatus() {
		$rs=$this->db_con->executeQuery("SELECT COUNT(*) AS admin FROM pgwWS_Admin");
		/* verify if true */
	...
	function createNewAdminAccount($username, $password, $ksize) {
		...
		$parameters = array (
			"arbitrary_data" => "<name>Payment Gateway</name><ip>".$ip."</ip>",
	...
	function createNewAdminAccount_($username, $password) {
		...
		$mycsr->createCSR(array("countryName" => 'PT', "stateOrProvinceName" => 'Lisboa', "localityName" => 'Lisboa', "organizationName" => 'ISCTE', "organizationalUnitName" => 'ADETTI', "commonName" => 'PGW', "emailAddress" => 'pgw@adetti.pt'),$private_key);
		...
		$parameters = array (
			"arbitrary_data" => "<name>Payment Gateway Server</name><ip>".$ip."</ip>",
		...
		/* inserts all data on the table */
		$sql = "INSERT INTO pgwws_admin VALUES ('$username', '".md5($password)."', '".$private_key->_spKey."', '".$result["certificate"]."')";
		$rs=$this->db_con->executeQuery($sql);
		if($rs==false) {
			$this->error = "ERROR: An error has occured while trying to insert data on the database.<br>";
			return false;
	...
	function getData() {
		$rs=$this->db_con->executeQuery("SELECT * FROM pgwWS_Admin");
		/* verificar resultado */
		if($rs->RecordCount()==0) return false;
		else {
	...
	function exportData() {
		CreateFile($_SERVER["DOCUMENT_ROOT"].$GLOBALS['INSTALLATION_DIR']."/PGW/admin/exports/publickey.xml", $this->pubkey->returnXMLKey());
		$aes = new AESEncrypt();
		CreateFile($_SERVER["DOCUMENT_ROOT"].$GLOBALS['INSTALLATION_DIR']."/PGW/admin/exports/privatekey.xml.enc", $aes->AES_encrypt($this->privkey->returnXMLKey(), $this->password));
		CreateFile($_SERVER["DOCUMENT_ROOT"].$GLOBALS['INSTALLATION_DIR']."/PGW/admin/exports/certificate.xml", $this->cert->returnXMLCertificate());
	}
...
<table width=\"500\" border=\"1\" cellspacing=\"0\" cellpadding=\"0\">
  ...
  <tr>
	...
	function checkPGWCert($cos_ausCert) {
		//$l = new Logger("PGW");
		...
		if($this->getData()==false) {
			...
			$C_COSAUS_cert = new Certificate();
			$C_COSAUS_cert->loadXMLcertificate($cos_ausCert);

			//$l->Log("PGW:PGW_Administrator:checkPGWCert:cos_ausCert: ".$cos_ausCert);

			$issuerPublicKey = new PublicKey();
			$issuerPublicKey->loadXMLkey($this->cert->getIssuerPublicKey());

			//$l->Log("PGW:PGW_Administrator:checkPGWCert:this->cert->getIssuerPublicKey(): ".$this->cert->getIssuerPublicKey());

			//$l->Log("PGW:PGW_Administrator:checkPGWCert:C_COSAUS_cert->getCertificateBody(): ".$C_COSAUS_cert->getCertificateBody());
			//$l->Log("PGW:PGW_Administrator:checkPGWCert:C_COSAUS_cert->getCertificateSignature(): ".$C_COSAUS_cert->getCertificateSignature());

			// verify signature of certificate $ausCert */
			if($GLOBALS['SECURITY_VAR']=="ON") {
				$sign_tool = new RSASignature();
				return $sign_tool->RSA_verify($C_COSAUS_cert->getCertificateBody(), $C_COSAUS_cert->getCertificateSignature(), $issuerPublicKey);
			} else {
				return true;
			}
		}
	...
	function verifyMessageSignature($message, $signature, $certificate) {
		//$l = new Logger("PGW");

		$cert = new Certificate();
		$cert->loadXMLcertificate($certificate);

		//$l->Log("PGW:PGW_Administrator:verifyMessageSignature:certificate: ".$certificate);

		$subjectPublicKey = new PublicKey();
		$subjectPublicKey->loadXMLkey($cert->getSubjectPublicKey());

		//$l->Log("PGW:PGW_Administrator:verifyMessageSignature:this->cert->getSubjectPublicKey(): ".$cert->getSubjectPublicKey());

		if($GLOBALS['SECURITY_VAR']=="ON") {
			$sign_tool = new RSASignature();
			return $sign_tool->RSA_verify($message, $signature, $subjectPublicKey);
		} else {
			return true;
		}
	}

	function verifySignature($identification, $message, $signature) {
		//$l = new Logger("PGW");
		//$l->Log("PGW:PGW_Administrator:verifySignature");

		$sql = "SELECT * FROM pgwws_cos WHERE identification='".$identification."'";
		//$l->Log("PGW:PGW_Administrator:verifySignature:this->sql:".$sql);

		$rs=$this->db_con->executeQuery($sql);
		if($rs==false) {
			return false;
		} else {
			return $this->verifyMessageSignature($message, $signature, $rs->fields[1]);
		}
	}

	function saveTransactionInformation($identification, $data) {
		//$l = new Logger("PGW");
		//$l->Log("PGW:PGW_Administrator:saveTransactionInformation");

		$this->getData();
		if($GLOBALS['SECURITY_VAR']=="ON") {
			$dec = new RSAEncrypt();
			$data_dec = $dec->RSA_decrypt($data, $this->privkey);
		} else {
			$data_dec = $data;
		}
		...
		$data_sql = htmlentities($data_dec);
		$sql = "INSERT INTO pgwws_transaction VALUES ('".$t_id."', '".$identification."', '".$data_sql."', 'PENDING')";
		//$l->Log("PGW:PGW_Administrator:saveTransactionInformation:this->sql:".$sql);

		if($this->db_con->executeQuery($sql)==false)
			return false;
		else
			return $t_id;
	}

	function modifyTransactionInformation($identification, $tid) {
		//$l = new Logger("PGW");
		//$l->Log("PGW:PGW_Administrator:modifyTransactionInformation");

		$sql = "UPDATE pgwws_transaction SET status='DONE' WHERE transaction_number='".$tid."' AND cos_id='".$identification."'";
		//$l->Log("PGW:PGW_Administrator:saveTransactionInformation:this->sql:".$sql);
	...
	function checkTransactionStatus($identification, $tid) {
		// we have to verify if the transaction ID exists and if it is in 'PENDING' state
		$sql = "SELECT * from pgwws_transaction WHERE transaction_number = '".$tid."' AND cos_id='".$identification."' AND status = 'PENDING'";
		$rs=$this->db_con->executeQuery($sql);
		if($rs->RecordCount()==0) return false;
		else return true;
	}

	function registerCOS($identification, $certificate) {
		//$l = new Logger("PGW");

		$sql = "INSERT INTO pgwWS_cos VALUES ('$identification', '$certificate', '')";

		//$l->Log("PGW:PGW_Administrator:registerCOS:sql: ".$sql);
