...
class CFS_Administrator extends DatabaseHandler
{
	var $db_con;
	 ...
	 */
	function CFS_Administrator()
	{
		//global $CFS_DB_SERVER, $CFS_DB_DBNAME, $CFS_DB_USRNAME, $CFS_DB_PWD;
		$this->db_con = new DatabaseHandler();
		$this->db_con->connectTo($GLOBALS['CFS_DB_SERVER'], $GLOBALS['CFS_DB_DBNAME'], $GLOBALS['CFS_DB_USRNAME'], $GLOBALS['CFS_DB_PWD']);
	}
	...
	function verifyLogin($username, $password){
		$rs=$this->db_con->executeQuery("SELECT * FROM ".$GLOBALS['CFS_DB_DBNAME'].".cfsws_admin WHERE login='$username' AND password='$password'");
		/* verificar resultado */
	...
	function verifyLogin_($username, $password){
		$rs=$this->db_con->executeQuery("SELECT * FROM ".$GLOBALS['CFS_DB_DBNAME'].".cfsws_admin WHERE login='$username' AND password='".md5($password)."'");
		/* verificar resultado */
		if($rs->RecordCount()==0) return false;
		return true;
	}
	...
	function verifyAdminStatus() {
		...
		//echo "SELECT COUNT(*) AS ".$GLOBALS['CFS_DB_DBNAME'].".admin FROM cfsws_admin";
		/* verify if true */
	...
	function createNewAdminAccount_($username, $password) {
		...
		$sql = "INSERT INTO ".$GLOBALS['CFS_DB_DBNAME'].".cfsws_admin VALUES ('$username', '".md5($password)."', '".$private_key->_spKey."', '".$result["certificate"]."')";
	...
	function createNewAdminAccount($username, $password, $ksize) {
		...
		$soapclient = new soapclient($AUS_LOCATION); // read from the configuration file
		...
		/* inserts all data on the table */
		$sql = "INSERT INTO ".$GLOBALS['CFS_DB_DBNAME'].".cfsws_admin VALUES ('$username', '$password', '".$this->pubkey->returnXMLkey()."', '".$this->privkey->returnXMLkey()."', '".$result["certificate"]."')";
		$rs=$this->db_con->executeQuery($sql);
		if($rs==false) {
			$this->error = "ERROR: An error has occured while trying to insert data on the database.<br>";
			return false;
	...
	function getData() {
		$rs=$this->db_con->executeQuery("SELECT * FROM ".$GLOBALS['CFS_DB_DBNAME'].".cfsws_admin");
		/* verificar resultado */
		if($rs->RecordCount()==0) return false;
		else {
...
";
	...
	function listServers_() {
		$rs=$this->db_con->executeQuery("SELECT * FROM ".$GLOBALS['CFS_DB_DBNAME'].".location");

		$servers = array();
		$n=0;
		while(!$rs->EOF) {
			$servers[$n] = $rs->fields[0]; $n++;
			$servers[$n] = $rs->fields[1]; $n++;
	...
	function addServer($sid, $location) {
	 ...
	 */
	function closeCFS_Administrator()
	{
