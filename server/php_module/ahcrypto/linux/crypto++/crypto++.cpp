...
extern "C" {
			...
			void * (*emalloc)(size_t))
	{
		...
		const CryptoPP::Integer & e = (priv.GetTrapdoorFunction()).GetPublicExponent();
		const CryptoPP::Integer & d = (priv.GetTrapdoorFunction()).GetPrivateExponent();
			...
			void * (*emalloc)(size_t))
	{
		...
		*cryptogram_len = pub.CiphertextLength(clear_data_len);
			...
			void * (*emalloc)(size_t))
	{
		...
		*clear_data_len = prv.Decrypt(rng, cryptogram, cryptogram_len, *clear_data).messageLength;
	}
			...
			void * (*emalloc)(size_t))
	{
		...
		return pub.VerifyMessage(data, data_len, signature, signature_len) ? 1 : 0;
	}
